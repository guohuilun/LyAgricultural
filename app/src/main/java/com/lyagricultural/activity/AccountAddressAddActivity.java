 ...
 */
public class AccountAddressAddActivity extends BaseActivity implements View.OnClickListener{
    private static final String TAG = "AccountAddressAddActivity";
     ...
     *  新增收货地址   -网络请求
     */
    private void initAccountAddressAdd(String isOk){
        if (CheckNetworkUtils.checkNetworkAvailable(this)){
            ...
            lecoOkHttpUtil.post().url(AppConstant.APP_USER_ADDRESS_CREATE)
                    ...
                    .addParams("adDetail",account_address_add_detailed_et.getText().toString().trim())
                    .addParams("zipCode","000000")
                    .addParams("isOk",isOk)
                    .build()
                    .execute(new StringCallback() {
                        ...
                        public void onResponse(String response) {
                            LyLog.i(TAG,"新增收货地址 = " +response);
                            Gson gson=new Gson();
                            DefaultBean parse=gson.fromJson(response,DefaultBean.class);
                            if ("OK".equals(parse.getStatus())){
                                LyToast.shortToast(AccountAddressAddActivity.this,parse.getMsg());
                                finish();
                            }else {
                                LyToast.shortToast(AccountAddressAddActivity.this,parse.getMsg());
                            }
...
//        task.setHideCounty(true);//隐藏县，显示省、市
        task.setCallback(new AddressPickTask.Callback() {
            ...
            public void onAddressInitFailed() {
                LyToast.shortToast(AccountAddressAddActivity.this, "数据初始化失败");
            }
            ...
            public void onAddressPicked(Province province, City city, County county) {
                ...
                if (county == null) {
                    ...
                    LyToast.shortToast(AccountAddressAddActivity.this, province.getAreaName() + city.getAreaName());
                } else {
                    address = province.getAreaName() + city.getAreaName() + county.getAreaName();
                    LyToast.shortToast(AccountAddressAddActivity.this, province.getAreaName() + city.getAreaName() + county.getAreaName());
                }
